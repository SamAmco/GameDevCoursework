...
void RenderObject::Draw() const 
{
	if(mesh) 
	{
		if (textures.size() > 0 && textures[0].getGLuint())
		{
			glUniform1i(glGetUniformLocation(shader->GetShaderProgram(), "tex1"), 0);
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, textures[0].getGLuint());
		}
		if (textures.size() > 1 && textures[1].getGLuint())
		{
			glUniform1i(glGetUniformLocation(shader->GetShaderProgram(), "tex2"), 1);
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, textures[1].getGLuint());
		}
		if (textures.size() > 2 && textures[2].getGLuint())
		{
			glUniform1i(glGetUniformLocation(shader->GetShaderProgram(), "tex3"), 2);
			glActiveTexture(GL_TEXTURE2);
			glBindTexture(GL_TEXTURE_2D, textures[2].getGLuint());
		}
		mesh->Draw();
	}
}